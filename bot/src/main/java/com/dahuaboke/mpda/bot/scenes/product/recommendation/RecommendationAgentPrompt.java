                ...
                , "tool", "。 根据工具执行后的结果,需要遵循以下规则" +
                        ...
                        "" + translate("recommendationTranslatePrompt") + """
                            如果是全量信息查询，则仅返回以下字段内容：
                                ...
                                七日年化(不加单位,当类型是货币式基金的情况，返回该字段)
                                万份收益(不加单位,当类型是货币式基金的情况，返回该字段)
                            如果是【配置】问题:
                                - 模型基于工具返回的结果,以及用户输出的需要分配的金额，将金额分配给不同类型基金的方案整理后给用户
                            如果是【推荐】问题:
                            ...
                            如果工具未返回任何信息，直接回复：未找到匹配条件的基金，您可以提供期望的年化利率、具体的基金类型、货基的万份收益或可接受的最大回撤率吗？这些信息将帮助我们更精准地筛选出符合您需求的基金产品。
                        """);
